...
import { User, CreditCard, Image as ImageIcon, Camera, CheckCircle2, Save } from "lucide-react";

export default function ConfiguracionPage() {
  const [activeTab, setActiveTab] = useState<"perfil" | "cobros">("perfil");

  // Estado Mock para Perfil
  const [perfil, setPerfil] = useState({
    nombreCanal: "Mi Estudio Cinemático",
    biografia: "Productora independiente creando contenido de alta calidad y reseñas de las mejores películas del mundo.",
    avatar: "",
    banner: ""
  });

  // Estado Mock para Cobros
  const [cobros, setCobros] = useState({
    metodoPreferido: "paypal",
    paypalEmail: "creador@ejemplo.com",
    cuentaBancaria: ""
  });

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`¡Cambios guardados con éxito en la pestaña de ${activeTab.toUpperCase()}! (Solo UI)`);
  };
    ...
    <div className="mx-auto max-w-4xl animate-in fade-in duration-500">
      ...
      <form onSubmit={handleSave} className="rounded-3xl border border-white/10 bg-[#0b0c15] shadow-xl overflow-hidden">
        ...
        {activeTab === "perfil" && (
          <div className="animate-in fade-in">
            ...
            <div className="relative h-48 w-full bg-gradient-to-r from-[#121826] to-[#0b0c15] border-b border-white/10">
              ...
              <button type="button" className="absolute right-4 top-4 flex items-center gap-2 rounded-lg bg-black/50 backdrop-blur px-3 py-1.5 text-xs font-medium text-white transition hover:bg-black/70 border border-white/10">
              ...
              <div className="absolute -bottom-10 left-8">
                <div className="group relative h-24 w-24 rounded-full border-4 border-[#0b0c15] bg-[#121826] overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex cursor-pointer items-center justify-center bg-black/60 opacity-0 transition-opacity group-hover:opacity-100">
              ...
              {cobros.metodoPreferido === 'paypal' ? (
                ...
                <div className="animate-in fade-in">
                  ...
                  <input 
                    ...
                    onChange={(e) => setCobros({...cobros, cuentaBancaria: e.target.value})}
        ...
        <div className="flex items-center justify-end gap-4 border-t border-white/10 bg-[#121826]/50 p-6">
          <button type="button" className="rounded-xl px-6 py-2.5 font-bold text-[#aeb4c0] transition hover:bg-white/5 hover:text-white">
            Descartar Cambios
          </button>
          <button type="submit" className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#00f2fe] to-[#4facfe] px-8 py-2.5 font-bold text-[#0b0c15] shadow-[0_0_15px_rgba(79,172,254,0.4)] transition hover:scale-105">
